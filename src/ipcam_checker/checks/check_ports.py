...
async def _check_tcp(ip: str, port: int, timeout: float) -> PortResult:
    ...
    try:
        ...
        _log.debug(
            "port.open", extra={"ip": ip, "port": port, "protocol": "tcp", "latency_ms": latency_ms}
        )
        ...
        _log.warning(
            "port.error", extra={"ip": ip, "port": port, "protocol": "tcp", "error": str(exc)}
        )
        return PortResult(port=port, protocol="tcp", open=False, error=str(exc))
